        ...
        while ( iter ) {
                if ( iter->corres) {
                        if ( iter->corres->verified != 0 ) { // Only verified ones get dumped.
                                char *c_id = get_uuid(PMPP_E_SERVER, iter->corres->plist);
                                
                                if ( !c_id )
                                        c_id = get_uuid(PMPP_E_SERVICE, iter->corres->plist);
             ...
             mlist )
        {
                ...
                if ( !*mlist ) {
                        *mlist = malloc(sizeof(*mlist));
                        ...
                        while ( iter->next ) {
                                if ( strcmp(m->m_hash, iter->msg->m_hash) == 0 ) // Exists.
                                        return 0;
...
int has_prefix(const char *str, const char *pre)
{
        ...
        return lenstr < len_pre ? 0 : strncmp(pre, str, len_pre) == 0;
}
